...
- **Term:** Tool | **Framework:** LangChain, CrewAI, smolagents | **What it actually is:** A function the LLM can call by name
- **Term:** Skill | **Framework:** Hermes, Semantic Kernel | **What it actually is:** Markdown instructions + optional scripts the agent loads on demand
- **Term:** Plugin | **Framework:** Semantic Kernel, ChatGPT | **What it actually is:** A tool bundled with a manifest
- **Term:** MCP Server | **Framework:** Anthropic MCP, Claude | **What it actually is:** A process that exposes tools over a standard protocol
- **Term:** Action | **Framework:** Composio, Arcade | **What it actually is:** A pre-built tool with auth already handled
...
- **Assumption:** Knows the data model | **Human developer:** Yes — reads docs first | **LLM caller:** No — guesses from the parameter name
- **Assumption:** Handles multi-step flows | **Human developer:** Composes calls in order | **LLM caller:** Skips steps, forgets IDs, hallucinates params
- **Assumption:** Reads error messages | **Human developer:** As debugging info | **LLM caller:** As literal instructions to follow
- **Assumption:** Retries on failure | **Human developer:** With backoff and reasoning | **LLM caller:** Immediately, same params, same result
- **Assumption:** Knows what it doesn'''t know | **Human developer:** Usually | **LLM caller:** Almost never
...
- ⚡ **2023 — Function calling lands** — OpenAI ships function calling. Agents can finally take real actions. Everyone wraps their APIs in a weekend.
- 📡 **Nov 2024 — MCP launches** — Anthropic publishes the Model Context Protocol. ~210 servers at launch. Most people ignore it.
- 🚀 **Mar 2025 — OpenAI adopts MCP** — OpenAI adds MCP support. Monthly downloads jump from 5M to 22M overnight. Now everyone pays attention.
- 🏆 **Q2 2025 — Protocol war over** — ChatGPT ships MCP. Cursor, Windsurf, Zed all add support in the same quarter.
- 🌐 **Q1 2026 — 9,400+ servers** — 92% of new agent frameworks ship MCP support by default. 78% of enterprise AI teams have it in production.
...
1. **Agent receives task**: Update dark mode for user@example.com
2. **Bad: multi-step**: get_user_id → hold ID in memory → get_prefs → update → verify. Agent drops context between steps.
3. **Good: single tool**: update_user_preference_by_email(email, key, val). Done.
4. **Confirmation returned**: Tool returns what changed. Agent reports back.
...
- **Bad name:** user_id | **Good name:** user_email_address | **Why:** Agent has the email. It never has the UUID.
- **Bad name:** store_id | **Good name:** store_location_name | **Why:** "downtown branch" works. "550e8400-..." never will.
- **Bad name:** msg | **Good name:** message_body_plain_text | **Why:** Removes any ambiguity about format
- **Bad name:** ts | **Good name:** event_timestamp_iso8601 | **Why:** Agent knows ISO 8601. Not your epoch.
- **Bad name:** type | **Good name:** notification_channel | **Why:** "email" or "sms" is obvious. "type: 2" is not.
...
1. **Task arrives**: "Deploy the new version of smart-wardrobe"
2. **Agent scans skill list**: Reads descriptions. deploy-service matches.
3. **Loads SKILL.md**: Full instructions, pitfalls, and exact commands now in context.
4. **Executes**: Follows numbered steps. The --user flag pitfall does not bite it.
5. **Verifies**: Checks logs. Reports back.
...
> Answer: Poor tool design — unclear descriptions, fragile multi-step chains. WildToolBench (2026) found tool design quality explains more variance in agent performance than model choice. A well-designed tool with a weaker model beats a poorly-designed tool with a stronger one.
