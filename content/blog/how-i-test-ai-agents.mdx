...
It's a CSV, a scoring script, and about two hours of your time the first week. There's no vector database or automated pipeline or fancy eval framework. I spent six months not doing it and learning from production failures instead. That's a much worse deal.
...
The jump from 63% to 79% didn't come from adding more test cases. It came from looking at failures and realizing that three simple prompt clarifications each fixed eight test cases. The failures are the product roadmap.
