...
**Example flow:** Agent requests access → access restricted to read-only → agent processes logs safely.
...
*(Quick check: why specify what an agent should NOT do? To prevent unexpected behavior, make debugging easier, and teach the agent boundaries.)*
...
1. ❌ Error occurs — an agent acts unexpectedly
2. 🔍 Check logs — trace the agent's actions
3. ✅ Fix issue — update code or constraints
