...
The MCP post I wrote last month mentioned a 200-query labeled test set. A few people asked about it. This is the whole thing.

It's not clever. I want to say that upfront. There's no vector database or automated pipeline or fancy eval framework. It's a CSV, a scoring script, and about two hours of your time the first week. The reason I'm writing this is that I spent six months not doing it and learning from production failures instead, which is a much worse deal.

---

## Why most evals tutorials miss the point

The internet has no shortage of posts about eval frameworks. They usually start with a big diagram: Prometheus as a judge, an LLM-as-judge setup with Gemini scoring another model's output, a whole orchestration layer around it. By paragraph three you're comparing Braintrust vs LangSmith vs a custom thing that someone's team built and is now maintaining forever.

I tried that path. I don't think it's wrong, but it answers the wrong question first. It answers "how do I *scale* testing?" before you've figured out "does this thing work at all?"

The number that matters first is brutally simple:

> on the 50 tasks I've actually asked this agent to do, how many did it get right?

You don't need a framework to answer that. You need a list and ten minutes.

---

## Step 0: know what "correct" means before you write a single test
...
## Step 2: the scoring script

Here's the actual script I use. It's under 100 lines:

```python
import csv, json, re, asyncio
...
## Step 3: the feedback loop that actually matters
...
<FlowDiagram
  steps='[
    ...
    {"label": "Fix one category at a time", "description": "If 8 failures are all the same root cause, fix that before touching the others"},
...
<Timeline
  events='[
    {"label": "Month 1 — No evals at all", "description": "Tested by hand. Shipped fixes. Broke other things. Did not notice for days."},
    {"label": "Month 3 — First labeled set (30 cases)", "description": "Scored 63%. Found three categories of failures that were obvious once I could see them."},
    {"label": "Month 4 — Grew to 100 cases", "description": "Added edge cases from real failures. Score: 79%. Much more useful."},
    {"label": "Month 6 — Automated in CI", "description": "Runs on every push to the agent prompt files. Blocks merge if score drops."},
    {"label": "Month 9 — 200 cases, 91%", "description": "Added an LLM judge for text quality. Only added it after the structural tests were solid."},
    {"label": "Now", "description": "New case added any time something breaks in production. The set grows with usage."}
  ]'
/>

The jump from 63% to 79% didn't come from adding more test cases. It came from looking at failures and realizing that three simple prompt clarifications fixed eight test cases each. The failures are the product roadmap.
...
<BarChart
  title="Where agent failures actually happen (production data)"
  data='[
...
Wrong tool or wrong params accounts for over half. This is a tool design problem more than a model problem. Which is actually good news — it's more fixable.

<Aside title="On eval set contamination">
Don't use your eval set cases as few-shot examples in your system prompt. I did this by accident in month two and my score jumped from 71% to 89% overnight. Felt great. Then I tried it on real user queries and it was just as bad as before. The number was lying.
...
In order of impact, from most to least, across my experience:
...
4. **Adjusting temperature** — often overlooked; lower temperature for routing/extraction, higher for text
5. **Model upgrade** — usually the last lever, not the first

<Quiz
  question="What's the fastest way to improve agent accuracy by 10+ points?"
  options='["Upgrade to a bigger model", "Add LLM-as-judge scoring", "Clarify the system prompt based on failure analysis", "Increase the size of the labeled eval set"]'
  answer="Clarify the system prompt based on failure analysis"
  explanation="In practice, prompt clarity explains more variance than model size for task-specific accuracy. The failures tell you exactly what to add. Bigger models help at the margin, but they don't fix vague instructions — they just fail more confidently."
/>
